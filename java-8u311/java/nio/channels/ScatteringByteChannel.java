...
/*
    表示一个可以分散（Scattering）多个ByteBuffer的通道，用于从一个通道中读取多个缓冲区的数据。
    ScatteringByteChannel接口提供了read方法，该方法可以将通道中的数据分散到多个ByteBuffer对象中。
    ScatteringByteChannel接口的实现类包括FileChannel和SocketChannel等。

    ByteBuffer buf1 = ByteBuffer.allocate(32);
    ByteBuffer buf2 = ByteBuffer.allocate(64);

    ScatteringByteChannel channel = new FileInputStream("input.txt").getChannel();

 ...
 */
/*
具体来说，GatheringByteChannel接口提供了write方法，该方法可以从多个ByteBuffer中聚集数据，并将聚集后的数据写入通道中。
GatheringByteChannel接口的实现类包括FileChannel和SocketChannel等。

ByteBuffer buf1 = ByteBuffer.allocate(32);
ByteBuffer buf2 = ByteBuffer.allocate(64);

// 将数据写入两个ByteBuffer对象中
GatheringByteChannel channel = new FileOutputStream("output.txt").getChannel();
...
channel.close();
在这个例子中，首先创建两个ByteBuffer对象buf1和buf2，并将数据写入这两个缓冲区中。
然后使用FileOutputStream获取文件输出流的通道，并将buf1和buf2作为ByteBuffer数组传递给write方法，实现了多个缓冲区的聚集写入。

需要注意的是，聚集操作只能用于将多个缓冲区的数据聚集到一个通道中。
如果需要从一个通道中读取多个缓冲区的数据，则需要使用ScatteringByteChannel接口。

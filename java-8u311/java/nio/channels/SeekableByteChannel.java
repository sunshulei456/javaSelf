 ...
 */
/*
    供了在通道中定位和操作特定位置数据的方法.
    接口的实现类包括FileChannel和SeekableByteChannel等

    position()：返回当前通道的位置。
    position(long newPosition)：设置当前通道的位置。
    size()：返回当前通道的大小。
    truncate(long size)：将当前通道截断为给定大小。
    force(boolean metaData)：将通道内容强制写入存储设备中。

    // === 案例
    SeekableByteChannel channel = Files.newByteChannel(Paths.get("test.txt"));

    ByteBuffer buffer = ByteBuffer.allocate(1024);

    // 使用position方法设置通道的位置为10，表示从文件的第10个字节开始读取数据。
    // 接下来，使用read方法从通道中读取数据到ByteBuffer对象中，最后使用flip方法切换ByteBuffer对象的读写模式，准备读取数据。
    channel.position(10);

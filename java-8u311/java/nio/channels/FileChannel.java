...
/*
    ...
    public static void main(String[] args) throws Exception {
        ...
        //从文件输入流获取通道
        FileChannel inputStreamChannel = new FileInputStream(file)
                                            .getChannel();
        //获取文件输出流 对应的管道
        FileChannel outputStreamChannel = new FileOutputStream(new File("2.txt"))
                                             .getChannel();
    ...
    implements SeekableByteChannel, GatheringByteChannel, ScatteringByteChannel
{
     ...
     */
    /**
     * 将当前 fileChannel 中的 [position, count) 区间中的 byte 内容写入 target 的缓存中
     * 可以将数据从当前通道传输到目标通道，而不需要手动读取当前通道的数据并写入目标通道。
     * 这种方法可以大大简化文件复制、网络传输等操作的实现。
     */
    /*
     ...
     */
    /**
     *  将 src_fileChannel 中的 [position, count) 区间中的 byte 内容写入当前 channel的缓存中
     */
    /*

    首先使用FileInputStream获取输入文件的通道inChannel，
    再使用FileOutputStream获取输出文件的通道outChannel。
    然后使用outChannel.transferFrom(inChannel, 0, inChannel.size())方法将输入文件的内容传输到输出文件中，
    最后关闭通道。
    -------------------------
    FileChannel inChannel = new FileInputStream("input.txt").getChannel();
    FileChannel outChannel = new FileOutputStream("output.txt").getChannel();

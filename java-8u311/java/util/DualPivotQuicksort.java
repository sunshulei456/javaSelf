...
/*
    快速排序是一种常用的排序算法，
    它的基本思想是选择一个基准元素，将数组分为两个部分，一部分小于基准元素，一部分大于基准元素，然后对这两个部分递归地进行排序。
    但是，快速排序在某些情况下会退化成 O(n^2) 的复杂度，
    例如数组已经有序或者基准元素选择不当的情况。为了解决这个问题，`DualPivotQuicksort` 使用了两个轴点，
    将数组分为三部分，可以更快地排序大型数组，并且避免了快速排序退化的情况。

    int[] arr = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5};
    DualPivotQuicksort.sort(arr, 0, arr.length - 1, null, 0, 0);
    System.out.println(Arrays.toString(arr)); // 输出：[1, 1, 2, 3, 3, 4, 5, 5, 5, 6, 9]
 */
/**
 *  <p>
 *      一种快速排序算法，被用于对数组进行排序。它的算法基于快速排序，但是使用了两个轴点（dual pivot），可以更快地排序大型数组。
 *  </p>
 *
